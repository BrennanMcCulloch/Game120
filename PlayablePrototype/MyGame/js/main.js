...
Stage3.prototype = {
	...
	create: function() {
		...
		ledge.scale.setTo(.90, 11);
	    ledge.body.immovable = true;
		ledge = platforms.create(game.world.width - 300, 200, 'ground'); // horizontal
		ledge.scale.setTo(.90, 11);
	    ledge.body.immovable = true;
		ledge = platforms.create(775, 0, 'wall'); // Right Wall
		ledge.body.immovable = true;
		ledge = platforms.create(-25, 0, 'wall'); // Left wall
		ledge.body.immovable = true;
		ledge = platforms.create(350, game.world.height/2, 'wall'); // gate 1 bottom
		ledge.body.immovable = true;
		ledge = platforms.create(450, game.world.height/2, 'wall'); // gate 2 bottom
		ledge.body.immovable = true;
		...
		//DOOR YOU CAN USE TO LEAVE
		door = game.add.sprite(game.world.width - 60, 128, 'door') // Add door
		door.scale.setTo(2, 2);
		door.anchor.setTo(0.5, 0.5);
		door.frame = 0; // Closed door frame
		doorCheck = false; //IN THIS STAGE, haven't picked up the weight yet
		...
		player = game.add.sprite(55, 115, 'bean', 'bean-float-00'); // The player and its settings
		makePlayer();
...
Stage4.prototype = {
	...
	create: function() {
		...
		player = game.add.sprite(185, 115, 'bean', 'bean-float-00'); // The player and its settings
		makePlayer();
